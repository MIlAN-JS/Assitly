(function () {
  ...
  function injectStyles(cfg) {
    ...
    const css = `
      ...
      /* ── Message rows ──
         padding: 0 6px keeps bubbles away from the panel's overflow:hidden edges */
      .assistly-msg {
        display: flex;
        gap: 8px;
      ...
      #assistly-send.active:hover { transform: scale(1.08); }
  ...
  function avatarHTML(cfg, size = 28) {
    if (cfg.botAvatar) {
      return `<img src="${cfg.botAvatar}" alt="${cfg.botName}" style="width:100%;height:100%;object-fit:cover;" />`;
    }
      ...
      <div id="assistly-panel">
        ...
        <div id="assistly-messages">
          <div class="assistly-msg bot">
            ...
            <div class="assistly-bubble assistly-welcome">${cfg.welcomeMessage}</div>
          </div>
        </div>

        <div id="assistly-input-area">
          <textarea id="assistly-input" rows="1" placeholder="${cfg.placeholder}"></textarea>
  ...
  async function sendMessage(text) {
    ...
    try {
      ...
      const data = await res.json();
      const reply = data?.message || data?.reply || data?.response || data?.content || "Sorry, I didn't get a response.";
  ...
  function attachEvents(root, cfg) {
    ...
    function closeWidget() { root.classList.remove("open"); }

    toggle.addEventListener("click", () => root.classList.contains("open") ? closeWidget() : openWidget());
    ...
    if (cfg.autoOpen) setTimeout(openWidget, 800);
  }
